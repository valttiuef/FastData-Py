...
<p>This help guide is organized around the workflows and concepts you use in FastData.</p>
<ul>
  <li><b>Basics</b> explains the core data and modeling concepts used across the app.</li>
  <li><b>Features</b> groups the main workflow areas and the controls available in each one.</li>
</ul>
<p>Use the navigation on the left to jump directly to the part of the application you are working with.</p>
...
<p>FastData lets you run and compare multiple models so you can choose the best fit based on your metrics and constraints.</p>
...
<ul>
  <li>Configure horizon and rolling-window validation settings for temporal data.</li>
...
<p>This feature exists in the codebase but is currently disabled in both development and release tab flags by default.</p>
<p>When enabled, use it for scenarios like demand planning, capacity prediction, or financial projections.</p>
...
<p>FastData uses forecasting models with time-series feature engineering and split strategies for experiments when the tab is enabled.</p>
...
<p>Chat history is stored per session.</p>
<ul>
  <li><b>New Chat</b> creates a separate empty session.</li>
  <li><b>Delete Chat</b> removes only the currently selected session.</li>
  <li>Other sessions and their history remain available in the session selector.</li>
</ul>
...
<p>Use the refresh icon next to Model name to fetch available models for the selected provider.</p>
...
<p>Choose how much model reasoning is shown in the assistant reply card.</p>
<ul>
  <li><b>Off</b>: hides reasoning output and focuses on final answers.</li>
  <li><b>Standard</b>: balanced reasoning visibility for normal use.</li>
  <li><b>High</b>: requests more extensive reasoning when the provider supports it.</li>
</ul>
<p>Provider behavior varies by model. If a model ignores this setting, chat still works and returns normal replies.</p>
...
<p>Use <b>Test</b> to save the key and verify it by fetching the available OpenAI models.</p>
