...
class RegressionTab(TabWidget):
    ...
    # @ai(gpt-5, codex-cli, enhancement, 2026-03-12)
    def _prediction_save_defaults(
        ...
        model_label = str(context.get("model_label") or "").strip()
        selector_label = str(context.get("selector_label") or "").strip()
        reducer_label = str(context.get("reducer_label") or "").strip()
        trained_at = str(context.get("trained_at") or "").strip()
        notes_lines = [
            "Generated by FastData Regression.",
            f"Target feature: {target_name}",
        ]
        if model_label:
            notes_lines.append(f"Model: {model_label}")
        if selector_label and selector_label.casefold() != "none":
            notes_lines.append(f"Feature selector: {selector_label}")
        if reducer_label and reducer_label.casefold() != "none":
            notes_lines.append(f"Dimensionality reduction: {reducer_label}")
        if trained_at:
            notes_lines.append(f"Trained at: {trained_at}")
        return {
            "name": default_name,
            "notes": "\n".join(notes_lines),
